...
def measure_state(state, num_meas):
    ...
    # Your code here #
    measurements = np.zeros(num_meas)
    for i in range(num_meas):
        measurements[i] = np.random.choice([0, 1], p=[abs(state[0])**2, abs(state[1])**2])
...
def quantum_algorithm(U):
    ...
    state_after_U = apply_u(U, initial_state)
    return measure_state(state_after_U, 20)
